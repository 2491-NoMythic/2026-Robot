...
public class Lights extends SubsystemBase {
  ...
  public Lights() {
    lights = new AddressableLED(LightConstants.LED_PWM_PORT);
    lights.setLength(LightConstants.ALL_LIGHT_END);
    LEDBuffer = new AddressableLEDBuffer(LightConstants.ALL_LIGHT_END);
    ...
    ledConfigs = ledConfigs.withBrightnessScalar(1).withStripType(StripTypeValue.RGB);
    featuresConfigs = featuresConfigs.withEnable5VRail(Enable5VRailValue.Enabled);

    candleConfig = candleConfig.withLED(ledConfigs).withCANdleFeatures(featuresConfigs);
